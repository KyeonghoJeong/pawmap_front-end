                ...
                if(error.response.status === 403){
                    ...
                    if(isNewAccessTokenLoaded){
                        try {
                            ...
                            // 200 => 재요청 성공
                            if(reGetMembersResponse.status === 200){
                                // 회원 정보, 총 페이지 수 저장
        ...
        async banMember(memberId, order){
            ...
            if(confirm("해당 회원을 "+word+"하시겠습니까?")){
                try {
                    ...
                    const banMemberResponse = await axios.put('http://localhost:8090/api/member/ban-date', {
                        memberId: memberId,
                        order: order,
                    }, {
                        headers: {'Authorization': `Bearer ${localStorage.getItem("accessToken")}`}
                    ...
                    // 200 => 요청 성공
                    if(banMemberResponse.status === 200){
                    ...
                    if(error.response.status === 403){
                        ...
                        if(isNewAccessTokenLoaded){
                            try {
                                ...
                                const reBanMemberResponse = await axios.put('http://localhost:8090/api/member/ban-date', {
                                    memberId: memberId,
                                    order: order,
                                }, {
                                    headers: {'Authorization': `Bearer ${localStorage.getItem("accessToken")}`}
                                ...
                                // 200 => 재요청 성공
                                if(reBanMemberResponse.status === 200){
                                    this.$router.go(this.$router.currentRoute); // 새로고침
                                }
                            } catch (error) {
                                console.log(error);
